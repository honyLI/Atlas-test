...
//         "/mnt/all.csv", "vendor_id", "tpep_pickup_datetime", "tpep_dropoff_datetime",
...
void print_number_vendor_ids_and_unique(StdDataFrame<uint64_t>& df)
{
    ...
    std::cout << "Number of vendor_ids in the train dataset: "
              << df.get_column<int>("vendor_id").size() << std::endl;
    std::cout << "Number of unique vendor_ids in the train dataset:"
              << df.get_col_unique_values<int>("vendor_id").size() << std::endl;
    std::cout << std::endl;
}
...
void calculate_distribution_store_and_fwd_flag(StdDataFrame<uint64_t>& df)
{
    ...
    auto unique_vendor_id_vec = Y_df.get_col_unique_values<int>("vendor_id");
    std::cout << '{';
...
void analyze_trip_timestamp(StdDataFrame<uint64_t>& df)
{
    ...
    df.multi_visit(std::make_pair("pickup_datetime", &max_visitor),
                   std::make_pair("pickup_datetime", &min_visitor));
