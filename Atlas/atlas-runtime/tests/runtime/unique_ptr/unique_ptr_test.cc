...
void test1(){
  ...
  int * p = (int *) malloc(1024 * 128);
  
  atlas_unique_ptr unique_ptr = atlas_make_unique_ptr(p, sizeof(int));
  int * get = (int *)atlas_up_deref_get(&unique_ptr);
  cout << get << endl;

//int * k = (int *)malloc(sizeof(int));
  atlas::AtlasUniquePtr<int> ptrs;
    //ptrs.reset_t(p);
    //int * get = ptrs.deref_get();
    //cout << get << endl;
